...
from command_helpers import add_item, remove_item, move_item, add_insult
...
def no_insult_input(update: Update, context: CallbackContext) -> int:
    try:
        add_insult(update, context)
        return ConversationHandler.END
    except ValueError:
        return ConversationStates.INSULT_INPUT_MISSING
...
def input_conversation_handler() -> ConversationHandler:
    ...
    input_conversation = ConversationHandler(
            ...
            states={
                    ...
                    ConversationStates.RM_INPUT_MISSING:     [
                            MessageHandler(filters=Filters.text, callback=no_rm_input)],
                    ConversationStates.MOVE_INPUT_MISSING:   [
                            MessageHandler(filters=Filters.text, callback=no_move_input)],
                    ConversationStates.INSULT_INPUT_MISSING: [
                            MessageHandler(filters=Filters.text, callback=no_insult_input)],
            },
